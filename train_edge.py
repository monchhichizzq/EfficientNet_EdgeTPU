...
arch_name      = "efficientnet_es" # "efficientnet_em_fusev2" # "efficientnet_es_fusev2"
model_name     = "efficientnet_es"

if model_name == "efficientnet_es":
    train_img_size, val_img_size = 160, 224
if model_name == "efficientnet_em":
    train_img_size, val_img_size = 172, 240
if model_name == "efficientnet_el":
    train_img_size, val_img_size = 224, 300

batch_size     = 256

# efficientnet_es, 224x224, 1.80 G, 5.44 M
# efficientnet_em, 240x240, 3.03 G, 6.90 M
# efficientnet_el, 300x300, 7.96 G, 10.59 M
...
group.add_argument('--data-dir', metavar='DIR', default="/home/zzq/Documents/data/imagenet_torch", #TODO
...
group.add_argument('--model', default=model_name, type=str, metavar='MODEL',
...
group.add_argument('--img-size', type=int, default=train_img_size, metavar='N',
                   help='Image size (default: None => model default)')
group.add_argument('--img-size-eval', type=int, default=val_img_size, metavar='N',
                   help='Image size (default: None => model default)')
...
group.add_argument('--channels-last', default=False, # action='store_true',  #TODO True
...
group.add_argument('--warmup-lr', type=float, default=1e-4, metavar='LR', #TODO
...
group.add_argument('--aa', type=str, default="rand-m6-mstd0.5-inc1",  #TODO
                    metavar='NAME', help='Use AutoAugment policy. "v0" or "original". (default: None)'),
...
group.add_argument('--bce-loss', default=True, #action='store_true'
                   help='Enable BCE loss w/ Mixup/CutMix use.')
group.add_argument('--bce-sum', action='store_true', default=False,
                   help='Sum over classes when using BCE loss.')
group.add_argument('--bce-target-thresh', type=float, default=0.2, #TODO
...
group.add_argument('--mixup', type=float, default=0.1,
...
group.add_argument('--smoothing', type=float, default=0.0, #0.1,
...
group.add_argument('--output', default=f'{arch_name}_input_{model_name}_A3', type=str, metavar='PATH',
...
def main():
    ...
    if arch_name == "efficientnet_es_fusev2":
        from models.efficientnet._efficientnet_model import efficientnet_es_fusev2
        model = efficientnet_es_fusev2(pretrained=False)

    if arch_name == "efficientnet_em_fusev2":
        from models.efficientnet._efficientnet_model import efficientnet_em_fusev2
        model = efficientnet_em_fusev2(pretrained=False)

    if arch_name == "efficientnet_el_fusev2":
        from models.efficientnet._efficientnet_model import efficientnet_el_fusev2
        model = efficientnet_el_fusev2(pretrained=False)

    if arch_name == "efficientnet_es":
        from models.efficientnet._efficientnet_model import efficientnet_es
        model = efficientnet_es(pretrained=False)

    if arch_name == "efficientnet_em":
        from models.efficientnet._efficientnet_model import efficientnet_em
        model = efficientnet_em(pretrained=False) 

    if arch_name == "efficientnet_el":
        from models.efficientnet._efficientnet_model import efficientnet_el
    ...
    val_data_size = (3, args.img_size_eval, args.img_size_eval)
    print("data_config['input_size']:", data_config['input_size'], val_data_size)
    if args.val_split:
        ...
        loader_eval = create_loader(
            dataset_eval,
            input_size=val_data_size, #data_config['input_size'],
    ...
    input_size = [3, val_img_size, val_img_size]
    example_inputs = torch.randn(1, *input_size).to(device)
    base_macs, base_params = tp.utils.count_ops_and_params(model, example_inputs)
    print("macs: {:.2f} G, params: {:.2f} M".format(base_macs/10**9, base_params/10**6))
