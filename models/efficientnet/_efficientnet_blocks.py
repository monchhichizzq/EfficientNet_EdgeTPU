...
__all__ = [
    ...
    'InvertedResidual', 'CondConvResidual', 'EdgeResidual', 'FuseV2',
...
class DepthwiseSeparableConv(nn.Module):
    ...
    def feature_info(self, location):
        if location == 'expansion':  # after SE, input to PW
            return dict(module='conv_pw', hook_type='forward_pre', num_chs=self.conv_dw.in_channels)
        else:  # location == 'bottleneck', block output
            return dict(module='', num_chs=self.conv_dw.out_channels)
...
class DepthwiseSeparableConv_MB(nn.Module):
    """ Depthwise-separable block
    Used for DS convs in MobileNet-V1 and in the place of IR blocks that have no expansion
    (factor of 1.0). This is an alternative to having a IR with an optional first pw conv.
    """
...
class InvertedResidual_MB(nn.Module):
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
        ...
        # Point-wise expansion
        self.conv_pw = create_conv2d(in_chs, mid_chs, exp_kernel_size, stride=1, padding=pad_type, **conv_kwargs)
        self.bn1 = norm_act_layer(mid_chs, inplace=True)
        ...
        # Point-wise (Fuse) linear projection
        print(f"(MB) exp_kernel_size: {exp_kernel_size}, dw_kernel_size: {dw_kernel_size}, pw_kernel_size: {pw_kernel_size}")
...
class FuseV2_v1(nn.Module):
    ...
    def __init__(
        ...
        # Point-wise expansion
        exp_kernel_size = 1
        self.conv_pw = create_conv2d(in_chs, mid_chs, exp_kernel_size, stride=stride, padding=pad_type, **conv_kwargs)
        self.bn1 = norm_act_layer(mid_chs, inplace=True)
        ...
        groups = 1 #2 
        pw_kernel_size = 3
        print(f"groups: {groups}, exp_kernel_size: {exp_kernel_size}, pw_kernel_size: {pw_kernel_size}")
        
        self.conv_pwl = create_conv2d(
            mid_chs, out_chs, pw_kernel_size, groups=groups, padding=pad_type, **conv_kwargs)
        self.bn3 = norm_act_layer(out_chs, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()
...
class EdgeResidual(nn.Module):
    """ Sparse FuseV2 Residual block with expansion convolution followed by pointwise-linear w/ stride
    ...
    def __init__(
        ...
        print(f"FuseV1, exp_kernel_size: {pw_kernel_size}, pw_kernel_size: {exp_kernel_size}, stride: {stride}")
...
class FuseV2(nn.Module):
    ...
    def __init__(
        ...
        # Point-wise linear projection
        self.conv_pwl = create_conv2d(mid_chs, out_chs, exp_kernel_size, padding=pad_type)
        self.bn2 = norm_act_layer(out_chs, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()

        print(f"FuseV2, exp_kernel_size: {pw_kernel_size}, pw_kernel_size: {exp_kernel_size}, stride: {stride}")
